...
def ID_hzd():
  ...
  if G_MEM.ID_EX_CTRL["MEM_READ"] == 1 and (
    G_MEM.ID_EX["RT"] == if_id_rs or G_MEM.ID_EX["RT"] == if_id_rt):
    G_MEM.FWD["PC_WRITE"] = 0
    G_MEM.FWD["IF_ID_WRITE"] = 0
    G_MEM.FWD["STALL"] = 1
  elif G_MEM.EX_MEM_CTRL["REG_WRITE"] == 1 and (
    G_MEM.ID_EX["RT"] == if_id_rs or G_MEM.ID_EX["RT"] == if_id_rt) and not G_UTL.data_fwding:
    G_MEM.FWD["PC_WRITE"] = 0
    G_MEM.FWD["IF_ID_WRITE"] = 0
    G_MEM.FWD["STALL"] = 1
  else:
